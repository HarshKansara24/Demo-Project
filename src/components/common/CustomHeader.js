...
class CustomHeader extends Component {
    ...
    _renderTitle() {
        ...
        if (title) {
            return (<Title style={{ color: titleColor || Colors.Defaultblack, alignSelf: 'flex-start', ...styles.titleStyle ,...titleStyle}}>{title}</Title>)
        } else if (image) {
        ...
        if (options) {
            ...
            if (icon) {
                ...
                return (<Clickable borderLess onPress={onPress}><Image source={image}
                                                                       style={{ marginHorizontal: 6, resizeMode: 'contain', height:30,width:30,...imageStyle }} /></Clickable>)
            } else if (text) {
